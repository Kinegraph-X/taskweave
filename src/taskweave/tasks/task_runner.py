...
class PoolTaskRunner:
    ...
    manager: WorkerPool
    # = field(default_factory= lambda : WorkerManager(log_bus = MiniBus(writer= StreamWriter(), observability_policy=ObservabilityPolicy.BEST_EFFORT, snapshot_getter = lambda: None)))
...
class SubprocessTaskRunner:
    # default manager for static type checking : not meant to be instanciated by default
    manager : WorkerPool = field(
        default_factory = lambda : SubProcessManager(
            source_id = "",
            log_bus = MiniBus(
                writer= StreamWriter(
